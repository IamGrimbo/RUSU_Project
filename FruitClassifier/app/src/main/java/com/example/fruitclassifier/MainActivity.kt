...
class MainActivity : AppCompatActivity(), SurfaceHolder.Callback, Camera.PictureCallback {
    ...
    private fun loadModelFromAsset(): ByteBuffer {
        ...
        val modelDescriptor = assetManager.openFd("fruit_classifier_model.tflite")
    ...
    private fun getPredictedClass(output: FloatArray): String {
        ...
        Log.d("getPredictedClass", "Output: ${output.joinToString(", ")}")

        val predictedClassIndex = output.indices.maxByOrNull { output[it] } ?: 0

        // Log the predicted class index and label
        Log.d("getPredictedClass", "Predicted Class Index: $predictedClassIndex")
        Log.d("getPredictedClass", "Predicted Class Label: ${classLabels[predictedClassIndex]}")
